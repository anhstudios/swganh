...
{    
    ...
    registration.CreateObject = [kernel] (swganh::plugin::ObjectParams* params) -> void * {
        return new StructureService(kernel);
    };

    registration.DestroyObject = [] (void * object) {
        if (object) {
			delete static_cast<StructureService*>(object);
        }
    };

    kernel->GetPluginManager()->RegisterObject("Structure::StructureService", &registration);    
