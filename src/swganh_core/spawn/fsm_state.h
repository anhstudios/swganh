...
public:
    ...
    std::map<EventType, Transition>& GetEventTransitions()
    {
        return event_transitions_;
    }
    std::multimap<boost::posix_time::time_duration, Transition>& GetTimedTransitions()
    {
        return timed_transitions_;
    }
