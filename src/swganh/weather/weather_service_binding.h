...
void exportSocialService()
{
	...
	class_<WeatherServiceInterface, shared_ptr<WeatherServiceInterface>, boost::noncopyable>("WeatherService", "The weather service processes in-game weather features.", no_init)
        .def("get_scene_weather", &WeatherServiceInterface::GetSceneWeather, "Returns the current weather ID from a specified scene.")
        .def("set_scene_weather", &WeatherServiceInterface::SetSceneWeather, "Sets the weather ID on a specified scene and broadcasts a ServerWeatherMessage to all players on that scene.")
        ;
