...
void Creature::AddContainedObject(shared_ptr<Object> object)
{
    {
	    boost::lock_guard<boost::mutex> lock(creature_mutex_);
		...
		if (arrangement_id == -1)
			GetSlotDescriptor()[-1]->insert_object(object);
		else
		{
			auto& arrangement = object->GetSlotArrangements()[arrangement_id-4];
			for (auto& i : arrangement)
			{
				auto replaced_object = GetSlotDescriptor()[i]->insert_object(object);
				// move replaced object to inventory if room				
			}
		}
...
void Creature::AddContainedObject(std::shared_ptr<Object> object, int32_t arrangement_id)
{
    {
	    boost::lock_guard<boost::mutex> lock(creature_mutex_);
		...
		// Add to first slot if can't find appropriate
		if (arrangement_id == -1)
			GetSlotDescriptor()[0]->insert_object(object);
		else
		{
			auto& arrangement = object->GetSlotArrangements()[arrangement_id-4];
			for (auto& i : arrangement)
