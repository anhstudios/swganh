...
uint32_t TangibleFactory2::GetType() const { return Tangible::type; }

 TangibleFactory2::TangibleFactory2(const shared_ptr<DatabaseManagerInterface>& db_manager)
    : ObjectFactory(db_manager)
{
}
void TangibleFactory2::LoadTemplates()
{
    try {
...
bool TangibleFactory2::HasTemplate(const string& template_name)
{
    auto found = GetTemplateIter_(template_name);
    return found != end(tangible_templates_);
}
unordered_map<string, shared_ptr<Tangible>>::iterator TangibleFactory2::GetTemplateIter_(const string& template_name)
{
...
void TangibleFactory2::PersistObject(const shared_ptr<Object>& object)
{
    if (object->IsDirty())
    {
        try 
...
void TangibleFactory2::DeleteObjectFromStorage(const shared_ptr<Object>& object)
{
    try 
    {
        auto conn = db_manager_->getConnection("galaxy");
...
shared_ptr<Object> TangibleFactory2::CreateObjectFromStorage(uint64_t object_id)
{
    auto tangible = make_shared<Tangible>();
    try {
        auto conn = db_manager_->getConnection("galaxy");
...
shared_ptr<Object> TangibleFactory2::CreateObjectFromTemplate(const string& template_name)
{
