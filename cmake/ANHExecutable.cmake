# ANHExecutable is a standardized way to build binary executables in
...
FUNCTION(AddANHExecutable name)
    PARSE_ARGUMENTS(ANHLIB "MMOSERVER_DEPS;ADDITIONAL_INCLUDE_DIRS;ADDITIONAL_SOURCE_DIRS;DEBUG_LIBRARIES;OPTIMIZED_LIBRARIES" "" ${ARGN})
    ...
    LIST(LENGTH ANHLIB_MMOSERVER_DEPS _project_deps_list_length)
    ...
    IF(_tests_list_length GREATER 0)        
        ...
        AddANHLibrary(${__project_library}
            MMOSERVER_DEPS
                ${ANHLIB_MMOSERVER_DEPS}
            SOURCES
                ${SOURCES}
            TEST_SOURCES
                ${TEST_SOURCES}
            ADDITIONAL_INCLUDE_DIRS
                ${ANHLIB_ADDITIONAL_INCLUDE_DIRS}
            DEBUG_LIBRARIES
                ${ANHLIB_DEBUG_LIBRARIES}
            OPTIMIZED_LIBRARIES
                ${ANHLIB_OPTIMIZED_LIBRARIES}
        )
    ...
    IF(_project_deps_list_length GREATER 0)
        TARGET_LINK_LIBRARIES(${name} ${ANHLIB_MMOSERVER_DEPS})
    ENDIF()
    
    IF(_debug_list_length GREATER 0)
        TARGET_LINK_LIBRARIES(${name} debug ${ANHLIB_DEBUG_LIBRARIES})
    ENDIF()
    
    IF(_optimized_list_length GREATER 0)
        TARGET_LINK_LIBRARIES(${name} optimized ${ANHLIB_OPTIMIZED_LIBRARIES})
    ENDIF()
    ...
    IF(WIN32)
        ...
        #    COMMAND call \"${PROJECT_SOURCE_DIR}/windows/postbuild.bat\" \"${PROJECT_SOURCE_DIR}\" \"${PROJECT_BINARY_DIR}\" \"\$\(ConfigurationName\)\"
        #)   
        
        # Create a custom built user configuration so that the "run in debug mode"
        # works without any issues.
    	#CONFIGURE_FILE(${PROJECT_SOURCE_DIR}/windows/user_project.vcxproj.in 
