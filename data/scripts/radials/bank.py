	...
	def handleRadial(self, owner, target, action):
		...
		if player_service:			
			if action == RadialIdentifier.bankItems:
				...
				transferBank(self, owner, target)
...
def joinBank(self, owner):
	...
	if bank:
		if bank.scene_id == owner.scene_id:
			...
			SystemMessage.sendSystemMessage(owner, swgpy.OutOfBand('system_msg', 'successfully_joined_bank'), False, False)
		else:
...
def quitBank(self, owner):
	...
	if bank:
		if bank.hasContainedObjects():
			bank.scene_id = 0
			SystemMessage.sendSystemMessage(owner, swgpy.OutOfBand('system_msg', 'successfully_quit_bank'), False, False)		
		else:
...
def transferBank(self, owner, target):
	...
	transferCallback = PythonCallback('radials.bank', 'transferCallback')
