...
-- First get our template_id
    ...
    IF does_exist > 0 THEN
      -- Now update our values
        ...
        custom_name = in_custom_name, volume = in_volume, arrangement_id = in_arrangement_id, permission_type = in_permission_type, type_id = in_object_type, updated_at = NOW()
    ...
    ELSE
    	INSERT INTO object (id, scene_id,parent_id, iff_template_id, x_position, y_position, z_position,  x_orientation, y_orientation, z_orientation, w_orientation,
    			...
    			values (in_object_id, in_scene_id,in_parent_id, iff_template, posX, posY, posZ, oX, oY, oZ, oW,
    				in_complexity, in_stf_name_file, in_stf_name_string, in_custom_name, in_volume, in_arrangement_id, in_permission_type, in_object_type, NOW(), NOW());
    END IF;
END//
DELIMITER ;
