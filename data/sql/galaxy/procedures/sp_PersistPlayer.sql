...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_PersistPlayer`(IN `in_object_id` BIGINT, IN `in_scene_id` INT, IN `in_parent_id` BIGINT, IN `template_string` VARCHAR(255), IN `posX` dOUBLE, IN `posY` dOUBLE, IN `posZ` DOUBLE, IN `oX` DOUBLE, IN `oY` DOUBLE, IN `oZ` DOUBLE, IN `oW` DOUBLE, IN `in_complexity` DOUBLE, IN `in_stf_name_file` VARCHAR(255), IN `in_stf_name_string` VARCHAR(255), IN `in_custom_name` VARCHAR(50), IN `in_volume` INT, IN `in_profession_tag` VARCHAR(255), IN `in_total_playtime` BIGINT, IN `in_csr_tag` SMALLINT, IN `in_max_force` INT, IN `in_experimentation_enabled` TINYINT, IN `in_crafting_stage` INT, IN `in_nearest_crafting_station` BIGINT, IN `in_experimentation_points` INT, IN `in_accomplishment_counter` INT, IN `in_current_language` INT, IN `in_current_stomach` INT, IN `in_max_stomach` INT, IN `in_current_drink` INT, IN `in_max_drink` INT, IN `in_jedi_state` INT)
BEGIN
